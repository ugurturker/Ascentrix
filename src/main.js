...
import { requestNotificationPermission, playNotifySuccess } from './modules/audio.js';
import { isFirebaseConfigured } from './modules/firebase.js';
import { onAuthChange, getCurrentUser } from './modules/auth.js';
import { enableCloudSync, disableCloudSync, saveUserData } from './modules/store.js';
...
// Init after DOM ready (legacy.js already loaded)
setTimeout(() => {
  updateAuthUI(getCurrentUser());
  onAuthChange((user) => {
    updateAuthUI(user);
    // Force legacy UI refresh after cloud sync
    try {
  ...
  // Watch for window event from auth.js
